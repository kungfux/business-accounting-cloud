    ...
    fastify.get(
        ...
        async function (request, reply) {
            ...
            if (list) {
                ...
                return await this.db.query('select * from properties where id in (?) order by title asc',
                    {
                        replacements: [ids],
            ...
            if (request.query.enabled) {
                return await this.db.query('select * from properties where companyId=? and enabled=? order by title asc limit ? offset ?',
                    {
            ...
            return await this.db.query('select * from properties where companyId = ? order by title asc limit ? offset ?',
                {
            ...
            const [result] = await this.db.query('insert into properties (title,inventoryNumber,cost,comment,enabled,companyId) ' +
                'values(?,?,?,?,?,?)',
                {
                    replacements: [
                        ...
                        request.body.inventoryNumber || null,
                        request.body.cost || null,
